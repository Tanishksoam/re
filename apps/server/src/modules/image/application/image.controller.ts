...
export class ImageController {
  ...
  @Post('/')
  async create(@Body() body: ImageCreateDto, @Req() request: Request) {
  ...
  @Get('/:imageId')
  async findOne(@Param('imageId') imageId: string, @Req() request: Request) {
    const queryOptions = RequestHelper.getQueryOptions(request)

    const item = await this.imageDomainFacade.findOneByIdOrFail(
      imageId,
      queryOptions,
    )
